...
class Ui_MainWindow(object):
    ...
    def callbackSymbolProgramOption(self,data):
        ...
        """
        msg.NormalTM2L7 = data.NormalTM2L7
        msg.DailyTM2L7 = data.DailyTM2L7
        msg.SuperQuickTM2L7 = data.SuperQuickTM2L7
        msg.Economy = data.Economy
        msg.Prewash = data.Prewash
        msg.Stain = data.Stain
        msg.ExtraRinse = data.ExtraRinse
        msg.Soft = data.Soft
        msg.Night_Cycle = data.Night_Cycle
        msg.Rinse_Hold = data.Rinse_Hold
        msg.Steam_Anticrease = data.Steam_Anticrease
        msg.Low_Steam = data.Low_Steam
        msg.Medium_Steam = data.Medium_Steam
        msg.Steam_Max = data.Steam_Max
        msg.Prewashing = data.Prewashing
        msg.Washing = data.Washing
        msg.Rinsing = data.Rinsing
        msg.Spinning = data.Spinning
        msg.Drying = data.Drying
        msg.Delay = data.Delay
        msg.Steaming = data.Steaming
        msg.Anticrease = data.Anticrease
        """
    ...
    def callbackRosMsg(self,data):
        if self.lcdTemp.value() and self.lcdTemp_2.value():
        ...
        #For Option
        if data.option == Options.aclarado.value:
            self.radioAclarado_2.setChecked(True)
        if data.option == Options.centrif.value:
            self.radioCentrif_2.setChecked(True)
        if data.option == Options.drenar.value:
            self.radioDrenar_2.setChecked(True)
        else:
            self.radioAclarado_2.setChecked(False)
            self.radioCentrif_2.setChecked(False)
            self.radioDrenar_2.setChecked(False)
            print("Error or not any option chosen!")

        if chkbxEconomy.isChecked() and chkbxEconomy_2.isChecked():
            chkbxEconomy_5.setChecked(True)


        rosmsg.NormalTM2L7 = data.NormalTM2L7
        rosmsg.DailyTM2L7 = data.DailyTM2L7
        rosmsg.SuperQuickTM2L7 = data.SuperQuickTM2L7
        rosmsg.Economy = data.Economy
        rosmsg.Prewash = data.Prewash
        rosmsg.Stain = data.Stain
        rosmsg.ExtraRinse = data.ExtraRinse
        rosmsg.Soft = data.Soft
        rosmsg.Night_Cycle = data.Night_Cycle
        rosmsg.Rinse_Hold = data.Rinse_Hold
        rosmsg.Steam_Anticrease = data.Steam_Anticrease
        rosmsg.Low_Steam = data.Low_Steam
        rosmsg.Medium_Steam = data.Medium_Steam
        rosmsg.Steam_Max = data.Steam_Max
        rosmsg.Prewashing = data.Prewashing
        rosmsg.Washing = data.Washing
        rosmsg.Rinsing = data.Rinsing
        rosmsg.Spinning = data.Spinning
        rosmsg.Drying = data.Drying
        rosmsg.Delay = data.Delay
        rosmsg.Steaming = data.Steaming
        rosmsg.Anticrease = data.Anticrease
        rosmsg.model = data.model
        rosmsg.temperature = data.temperature
        rosmsg.Spin_speed = data.Spin_speed
        rosmsg.User_select_Rinse_Nums = data.User_select_Rinse_Nums
        rosmsg.User_Select_Delay_Time = data.User_Select_Delay_Time
        rosmsg.User_Select_Program_ID = data.User_Select_Program_ID
        """
    ...
    def setupUi(self, MainWindow):
        ...
        brush = QtGui.QBrush(QtGui.QColor(255,255,255)) #instantiate colour
    ...
    def showInfo(self):
        if not rospy.is_shutdown():
            coordinate_sub_kg = rospy.Subscriber("kg_info", screen, self.callbackKg)
            coordinate_sub_temp = rospy.Subscriber("temp_info", screen, self.callbackTemp)
            coordinate_sub_centr = rospy.Subscriber("centrif_info", screen, self.callbackCentr)
            coordinate_sub_speed = rospy.Subscriber("speed_info", screen, self.callbackSpeed)
            coordinate_sub_hour = rospy.Subscriber("hour_info", screen, self.callbackHour)
