...
"""
# Designing PointCloud2 message
pcloud2_msg.header.frame_id = "refObject"
pcloud2_msg.header.seq = 1
pcloud2_msg.header.stamp = rospy.Time.now()
pcloud2_msg.height = cloud_rows
pcloud2_msg.width = cloud_cols
pcloud2_msg.point_step = ref_obj_array.dtype.itemsize
pcloud2_msg.row_step = pcloud2_msg.point_step * cloud_cols
pcloud2_msg.data = ref_obj_array.tostring()
# Designing PointCloud message
pcloud_msg.header.frame_id = "clouddata"
 pcloud_msg.header.stamp = rospy.Time.now()
pcloud_msg.header.seq = 1

for i in range(cloud_rows):
p32 = Point32()
p32.x = ref_obj_array[i,0]
p32.y = ref_obj_array[i,1]
p32.z = ref_obj_array[i,2]
pcloud_msg.points.append(p32)
"""


#CRF_projections = np.array([P_x,P_y,P_z,1.0], dtype = np.float32)
#try:
#    transForHeight = self.tfBufferHeight.lookup_transform('base_footprint','xtion_rgb_optical_frame',rospy.Time())
#    P_base = globalVars.findBaseProjections(transForHeight, CRF_projections)
#except tf2_ros.LookupException as e:
#    print(e)
            #print("x: %f y: %f z: %f" %(P_base[0], P_base[1], P_base[2]))
